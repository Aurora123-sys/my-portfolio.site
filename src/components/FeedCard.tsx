...
const ACCENT_BORDER: Record<Project["accent"], string> = {
  ...
  violet: "hover:border-violet2/60",
  cyan: "hover:border-cyan2/60",
  amber: "hover:border-amber2/60",
};
...
export default function FeedCard({ project }: { project: Project }) {
  ...
  return (
    <motion.article
      ...
      className={clsx(
        "feed-card group relative overflow-hidden rounded-[1.75rem] border border-line bg-white shadow-bento transition-all duration-500",
    ...
    >
      ...
      <header className="flex items-center justify-between gap-3 px-6 pt-5">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1 text-[0.92rem] font-medium text-ink-900">
            ...
            <div className="font-mono text-[0.7rem] uppercase tracking-[0.08em] text-ink-500">
      ...
      <div className="relative mt-4 aspect-[16/10] overflow-hidden">
        ...
        >
          <div className="flex h-14 w-14 items-center justify-center rounded-full bg-white/85 text-ink-900 shadow-bento backdrop-blur-md">
      ...
      <div className="px-6 pt-5 pb-6">
        <h3 className="font-display text-[1.45rem] leading-tight tracking-tight text-ink-900">{project.title}</h3>
        <p className="mt-2 text-[0.94rem] leading-relaxed text-ink-500">{project.blurb}</p>
        ...
        <footer className="mt-6 flex items-center justify-between border-t border-line pt-4 text-sm text-ink-500">
          <div className="flex items-center gap-5">
            <span className="flex items-center gap-1.5"><Heart /> {project.reactions.likes}</span>
            <span className="flex items-center gap-1.5"><Save /> {project.reactions.saves}</span>
            <span className="hidden items-center gap-1.5 sm:flex">
              <span className="num text-ink-900">{project.stats[0]?.value}</span>
              <span className="font-mono text-[0.7rem] uppercase tracking-[0.08em] text-ink-400">{project.stats[0]?.label}</span>
            </span>
          </div>
          <Link href="/work" className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[0.85rem] text-ink-900 hover:bg-paper-200">
            Open
  ...
  return (
    <div className="absolute bottom-3 left-3 flex items-end gap-0.5">
      {[0, 1, 2, 3].map((i) => (
        <motion.span
          key={i}
          className="w-[3px] rounded-sm bg-cobalt"
          animate={active ? { height: [6, 14, 6, 18, 10, 6][i] ?? 8, opacity: 1 } : { height: 4, opacity: 0.4 }}
          transition={{ duration: 0.5, repeat: active ? Infinity : 0, repeatType: "mirror", delay: i * 0.07 }}
        />
      ))}
    </div>
  );
...
// keep export for parity
export const _accentToRing = accentToRing;
